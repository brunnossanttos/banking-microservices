...
resource "aws_security_group" "app" {
  ...
  ingress {
    description     = "Clients service from ALB"
    from_port       = 3001
    to_port         = 3001
    protocol        = "tcp"
  ...
  ingress {
    description     = "Transactions service from ALB"
    from_port       = 3002
    to_port         = 3002
    protocol        = "tcp"
    security_groups = [aws_security_group.alb.id]
...
resource "aws_security_group" "database" {
  ...
  ingress {
    description     = "RabbitMQ from app servers"
    from_port       = 5671
    to_port         = 5671
    protocol        = "tcp"
    security_groups = [aws_security_group.app.id]
...
resource "aws_db_instance" "clients" {
  identifier = "${var.project_name}-clients-db"
  ...
  tags = {
    Name = "${var.project_name}-clients-db"
  }
}

resource "aws_db_instance" "transactions" {
  identifier = "${var.project_name}-transactions-db"
  ...
  tags = {
    Name = "${var.project_name}-transactions-db"
  }
}
...
resource "aws_lb_target_group" "transactions" {
  name     = "${var.project_name}-transactions-tg"
  port     = 3002
  protocol = "HTTP"
  vpc_id   = aws_vpc.main.id
...
resource "aws_lb_target_group_attachment" "clients" {
  target_group_arn = aws_lb_target_group.clients.arn
  target_id        = aws_instance.app.id
  port             = 3001
}

resource "aws_lb_target_group_attachment" "transactions" {
...
resource "aws_cloudwatch_log_group" "clients" {
  name              = "/banking/clients-service"
  retention_in_days = 30

  tags = {
    Name = "${var.project_name}-clients-logs"
  }
}

resource "aws_cloudwatch_log_group" "transactions" {
  name              = "/banking/transactions-service"
  retention_in_days = 30

  tags = {
    Name = "${var.project_name}-transactions-logs"
  }
}
